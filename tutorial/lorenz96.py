...
def plot_l96_vars_polar(X,Y,Z, nvars):
    ...
    fig, axs = plt.subplots(subplot_kw={'projection':'polar'}) 
    axs.plot(X_theta,X[-1,:,0,0], label='X')
    axs.plot(Y_theta,np.ravel(Y[-1,:,:,0]), label='Y')
    axs.plot(Z_theta,np.ravel(Z[-1,:,:,:]), label='Z')
...
def main():
    ...
    L96 = Lorenz96_three(K=K,J=J,I=I,h=h,g=g,b=b,c=c,e=e,d=d,F=F,nlevels=3)
    ...
    for i in range(npoints):
        ...
        # x_,y_,z_ = L96.rk4_step(dt,np.stack((x,y,z)))
        xyz_ = L96.rk4_step(dt,xyz)
