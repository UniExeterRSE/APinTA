...
INT_FUNC_TYPE = Callable[
    [np.ndarray, float, float, float, np.ndarray, float, Callable],
    np.ndarray]
BURGERS_INT_LIST: List[INT_FUNC_TYPE] = [burgers.burgers_explicitRK, burgers.burgers_imexRK]
BURGERS_INT_NAMES: List[str] = ['RK', 'IMEX']
...
class BurgersParareal(FixedParareal):
    def __init__(self, t_range: Tuple[float, float], dt_fine: float, dt_coarse: float,
                 ...
                 iterations: int, nu: float, coarse_int_func: INT_FUNC_TYPE):
    ...
    def fine_integration_func(self, t_vals: List[float], x_in: np.ndarray) -> List[np.ndarray]:
        ...
        for i, t in enumerate(t_vals[:-1]):
            ...
            output[i+1] = burgers.burgers_imexRK(output[i], self.dt_fine, self.dx, self.nu, self.x_vals, t, self.q_func)
            
        return list(output)
        
    def coarse_integration_func(self, a: float, b: float, x_in: np.ndarray) -> np.ndarray:
        return self.coarse_int_func(x_in, b-a, self.dx, self.nu, self.x_vals, a, self.q_func)
    
def serial_solve(x_vals, t_vals, nu, x0, q_func, integ_func: INT_FUNC_TYPE):
    ...
    output[0, :] = x0
    for i, t in enumerate(t_vals[:-1]):
        output[i+1, :] = integ_func(output[i, :], dt, dx, nu, x_vals, t, q_func)
        
    return output

def serial_solves(x_range, t_range, dx_vals, dt_vals, nu_vals, integ_func: INT_FUNC_TYPE,
                  sol_func: Callable, q_func: Callable):
...
if __name__ == '__main__':
    ...
    serial_solves(x_range, t_range, [1/64, 1/128, 1/256], [1e-4, 1e-3, 1e-2], [0]+[10**x for x in range(-4,1)], burgers.burgers_imexRK, B1_sol, B1_q)
